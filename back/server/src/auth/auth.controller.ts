...
export class AuthController {
  ...
  @Patch('logout')
  @ApiOperation({ summary: '로그아웃 API', description: '로그아웃을 시도한다.' })
  async logout() {
  ...
  @Redirect('https://api.intra.42.fr/oauth/authorize?client_id=afc0a3e907f953c39d371c4fee3fd72b29b320fbf95da01448b787745a0e066c&redirect_uri=http%3A%2F%2F10.19.226.233%3A3000%2Foauth%2Flogin&response_type=code', 302)
  @ApiOperation({ summary: '42 Oauth API 로그인', description: '42 Oauth API에 로그인을 시도한다.' })
  @Get('42')
  getOauthPage() {
  ...
  @Post('sendEmail')
  @ApiOperation({ summary: '2차 인증 이메일 전송 API', description: '2차 인증을 위해 이메일을 전송한다.' })
  async sendEmail(
    @Query('id') id: string,
  ...
  @Post('validEmail')
  @ApiOperation({ summary: '2차 인증 이메일 확인 API', description: '2차 인증을 위해 이메일로 전송된 코드를 확인한다.' })
  async validEmail(
    @Query('id') id: string,
