...
export class BanService {
	...
	async create(createBanDto: CreateBanDto) {
		...
		if (ban1 === null)
			throw new HttpException(`${createBanDto.player1}: Cannot find user`, HttpStatus.BAD_REQUEST);
		if (ban2 === null)
			throw new HttpException(`${createBanDto.player2}: Cannot find user`, HttpStatus.BAD_REQUEST);
	...
	async getBanListOne(nickName: string) {
		...
		let resFrinedListDto = new ResBanListDto();
