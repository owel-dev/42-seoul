...
template <class T, class Allocator = std::allocator<T> > class vector
{
  ...
  public:
    ...
    vector(const vector &copy) : _alloc(copy._alloc), _begin(nullptr), _end(nullptr), _end_cap(nullptr)
    {
    ...
    template <class InputIterator> iterator insert(InputIterator position, size_type n, const value_type &val)
    {
        size_type d = position - _begin;
        if (_end_cap < _end + n)
        {
